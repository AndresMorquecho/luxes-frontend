...
export function InstalacionPanel({ proyectoId }) {
  ...
  return (
    <div className="space-y-3 sm:space-y-5">
      ...
      {/* KPI Cards Grid — 2 columnas en móvil para reducir scroll */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 lg:gap-4">
        {/* Status Card */}
        <div className="bg-slate-50 border border-slate-200/60 rounded-xl p-2.5 sm:p-4 flex items-center gap-2 sm:gap-3 min-w-0">
          <div className={`w-7 h-7 sm:w-9 sm:h-9 rounded-lg flex items-center justify-center shrink-0 border ${
          ...
          <div className="min-w-0">
            <span className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-wider block truncate">Estado de Obra</span>
        ...
        {/* Start Date Card */}
        <div className="bg-slate-50 border border-slate-200/60 rounded-xl p-2.5 sm:p-4 flex items-center gap-2 sm:gap-3 min-w-0">
          <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-lg bg-slate-100 text-slate-500 border-slate-200/40 flex items-center justify-center shrink-0">
        ...
        <div className="bg-slate-50 border border-slate-200/60 rounded-xl p-2.5 sm:p-4 flex items-center gap-2 sm:gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <span className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-wider block truncate">Equipo Técnico</span>
            <span className="text-[11px] sm:text-xs font-bold text-slate-800 mt-0.5 block truncate">
        ...
        <div className="bg-slate-50 border border-slate-200/60 rounded-xl p-2.5 sm:p-4 flex items-center gap-2 sm:gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <span className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-wider block truncate">Requisitos</span>
            <span className="text-[11px] sm:text-xs font-bold text-slate-800 mt-0.5 block truncate">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-6">
        ...
        <div className="lg:col-span-2 border border-slate-200/80 rounded-xl bg-white shadow-sm overflow-hidden flex flex-col justify-between">
          ...
          <div className="p-4 sm:p-6 border-b border-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-6">
              <div>
                ...
                <p className="text-sm font-semibold text-slate-700 mt-1 leading-snug sm:leading-relaxed">
            ...
            {datosInstalacion.notas && (
              <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-slate-100">
                ...
                <p className="text-xs text-slate-600 bg-slate-50/50 p-2.5 sm:p-3 rounded-lg border border-slate-100 whitespace-pre-line leading-relaxed">
                  {datosInstalacion.notas}
                </p>
              </div>
            )}
            ...
            {/* Móvil: validación y equipo en acordeón para no duplicar scroll */}
            <div className="lg:hidden mt-3 pt-3 border-t border-slate-100">
              <button
                type="button"
                onClick={() => setDetallesMobileOpen((p) => !p)}
                className="w-full flex items-center justify-between gap-2 text-left py-1"
              >
                <span className="text-xs font-bold text-slate-600 uppercase tracking-wider">
                  Validación y equipo ({metCount}/{totalCount})
                </span>
                <span className={`text-slate-400 transition-transform duration-200 ${detallesMobileOpen ? 'rotate-180' : ''}`}>
                  ...
                  {personalAsignado.length > 0 && (
                    <div className="pt-2 border-t border-slate-100 space-y-2">
                      {personalAsignado.map((p) => (
                        <div key={p.empleadoId} className="flex items-center gap-2">
                          <div className="w-6 h-6 rounded-md bg-indigo-50 text-indigo-600 font-bold text-[9px] flex items-center justify-center border border-indigo-100/40 shrink-0">
                            {p.nombre.substring(0, 2).toUpperCase()}
                          </div>
                          <span className="text-xs font-semibold text-slate-750 truncate">{p.nombre}</span>
                        </div>
                      ))}
                    </div>
                  )}
          ...
          <div className="p-3 sm:p-4 bg-slate-50/30 border-t border-slate-100 flex-1 flex flex-col">
            {(() => {
        ...
        {/* Right Side: Checklist & Assigned Team — solo desktop */}
        <div className="hidden lg:flex border border-slate-200/80 rounded-xl bg-white shadow-sm overflow-hidden flex-col justify-between">
          <div>
