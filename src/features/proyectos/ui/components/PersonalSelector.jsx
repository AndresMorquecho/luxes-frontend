  ...
  return (
    <div className="space-y-6">
      ...
      {personalAsignado.length > 0 && (
        <div className="overflow-x-auto border border-slate-200 rounded-xl bg-white shadow-sm mobile-table-cards">
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-slate-50 text-slate-500 border-b border-slate-200">
                ...
                {!soloLectura && <th className="p-3 font-bold uppercase tracking-wider text-center" style={{ width: '80px' }}>Acción</th>}
              </tr>
            </thead>
            <tbody>
              {personalAsignado.map((p) => (
                <tr key={p.empleadoId} className="border-b border-slate-100 text-slate-600 hover:bg-slate-50/50">
                  <td className="p-3" data-label="Nombre / Cargo">
                    <div className="font-bold text-slate-800">{p.nombre}</div>
                    <div className="text-[10px] text-slate-400">{p.cargo || 'Personal'}</div>
                  </td>
                  <td className="p-3" data-label="Rol de Instalación">
                    {soloLectura ? (
                      <span className="font-bold text-slate-700">{p.rol}</span>
                    ) : (
                      <select
                        value={p.rol}
                  ...
                  <td className="p-3" data-label="Observaciones">
                    {soloLectura ? (
                      <span className="text-slate-600 italic">{p.notes || p.notas || 'Sin observaciones'}</span>
                    ) : (
                      <input
                        type="text"
                  ...
                  {!soloLectura && (
                    <td className="p-3 text-center" data-label="Acción">
                      <button 
