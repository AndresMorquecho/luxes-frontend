...
function DonutChart({ data, title }) {
  ...
  return (
    <div className="flex flex-col items-center p-6 bg-white rounded-xl border border-slate-200 w-full">
...
function TrendChart({ data, title, keys, colors, labels }) {
  ...
  return (
    <div className="bg-white p-6 rounded-xl border border-slate-200 w-full space-y-4">
                ...
                {pts.map((p, pIdx) => (
                  <g key={pIdx} className="group/dot cursor-pointer">
                    ...
                    <text
                      ...
                      textAnchor="middle"
                      fill="white"
                      className="text-[7px] font-bold opacity-0 group-hover/dot:opacity-100 transition-opacity duration-200 pointer-events-none"
                    >
      ...
      {hoveredBar && (
        ...
        >
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-white font-bold">${Math.round(hoveredBar.value).toLocaleString()}</span>
          </div>
        </div>
      )}
    </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6 justify-center">
          ...
          <div className="relative w-36 h-36 shrink-0">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none text-center">
              <span className="text-[8px] text-slate-400 font-semibold uppercase tracking-widest">{centerText || 'Total'}</span>
              <span className="text-base font-bold text-slate-800 leading-none mt-0.5">
              ...
              return (
                <div key={idx} className="flex items-center justify-between text-xs py-1.5 border-b border-slate-50 last:border-none">
                  ...
                  <div className="text-right flex items-center gap-3">
                    <span className="font-semibold text-slate-800">${Math.round(item.value).toLocaleString()}</span>
...
function PagarComboChart({ data }) {
  ...
  // Escala mínima legible cuando no hay montos
  const yMax = hasData ? dataMax * 1.2 : 1000;
    ...
    <div className="relative w-full overflow-hidden" onMouseMove={handleMouseMove}>
            ...
            return (
              <g key={idx}>
                <line
                  ...
                  strokeDasharray="4 4"
                />
                {/* Sin datos: no pintar etiquetas (evita la franja oscura $0/$1/$1) */}
                {hasData && (
                  <text
                    x={paddingLeft - 8}
                    y={y + 3}
      ...
      {hoveredPoint && (
        ...
        >
          ...
          <span className="text-white font-bold">{hoveredPoint.value}</span>
        </div>
      )}
    </div>
  );
...
export function BalancesPage() {
  ...
  const renderGrowthIndicator = (pct, colorClass) => {
    ...
    return (
      <span className={`text-[11px] font-medium ${colorClass}`}>
        {isPositive ? '+' : '−'}{absPct}% vs año anterior
      </span>
    );
  };
  ...
  if (loading) {
    return (
      <div className="space-y-3 sm:space-y-5 animate-slide-up balances-page" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
        <div className="bg-white shadow-card rounded-xl border border-gray-100 flex flex-col items-center justify-center py-20 gap-3">
          <div className="animate-spin rounded-full h-6 w-6 border-2 border-blue-200 border-t-blue-500" />
          <p className="text-slate-500 text-sm font-medium">Consolidando estados financieros...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3 sm:space-y-5 animate-slide-up balances-page" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      <style>{`
        .balances-page, .balances-page * { font-family: 'Inter', system-ui, sans-serif; box-sizing: border-box; }
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <h1 className="text-xl font-bold text-slate-800">Balances</h1>
              ...
              <p className="text-sm text-slate-500 mt-0.5">
                Rentabilidad, cuentas por cobrar/pagar e índices de satisfacción
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="relative">
              <select
                ...
                className="appearance-none bg-slate-50 hover:bg-white border border-slate-200 text-slate-700 text-sm font-semibold px-4 py-2.5 pr-9 rounded-xl cursor-pointer focus:outline-none focus:border-blue-300 focus:ring-2 focus:ring-blue-100 transition-all"
              >
      ...
      {/* Tabs */}
      <div className="bg-white shadow-card border border-gray-100 rounded-xl p-1.5 flex justify-center overflow-x-auto">
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-semibold transition-all whitespace-nowrap ${active
      ...
      <div className="bg-white shadow-card border border-gray-100 p-1 rounded-xl">

        {activeTab === 'resumen' && (
          <div className="flex flex-col gap-4 p-3 sm:p-4 animate-fadeIn">
            <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
              <div className="px-4 sm:px-5 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100">
                <div>
                  <h3 className="text-sm font-semibold text-slate-800">Flujo mensual</h3>
                  <p className="text-xs text-slate-500 mt-0.5">
                    Ingresos vs egresos · {formatPeriodLabel(desde, hasta)}
                  </p>
                </div>
                <div className="relative shrink-0">
                  <select
                    ...
                    className="appearance-none bg-slate-50 border border-slate-200 text-slate-700 text-sm font-semibold px-3 py-2 pr-8 rounded-xl cursor-pointer focus:outline-none focus:border-blue-300 focus:ring-2 focus:ring-blue-100"
                  >
                    <option value={2026}>2026</option>
                    <option value={2025}>2025</option>
                    <option value={2024}>2024</option>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-slate-100 border-b border-slate-100">
                <div className="px-4 sm:px-5 py-4">
                  <span className="text-xs text-slate-500 font-medium">Ingresos totales</span>
                  <p className="text-lg font-bold text-slate-800 mt-1">{fmt(ingresosTotal)}</p>
                  {renderGrowthIndicator(ingresosGrowth, 'text-emerald-600')}
                </div>
                <div className="px-4 sm:px-5 py-4">
                  <span className="text-xs text-slate-500 font-medium">Egresos totales</span>
                  <p className="text-lg font-bold text-slate-800 mt-1">{fmt(egresosTotal)}</p>
                  {renderGrowthIndicator(egresosGrowth, 'text-amber-700')}
                </div>
                <div className="px-4 sm:px-5 py-4">
                  <span className="text-xs text-slate-500 font-medium">Resultado neto</span>
                  <p className="text-lg font-bold text-slate-800 mt-1">{fmt(netTotal)}</p>
                  {renderGrowthIndicator(netGrowth, 'text-blue-600')}
                </div>
              </div>

              <div className="px-4 sm:px-5 pt-4 flex items-center justify-center gap-5 text-xs text-slate-500 font-medium">
                <div className="flex items-center gap-1.5">
              ...
              {loadingFlujo ? (
                <div className="h-52 flex flex-col items-center justify-center gap-2">
                  <div className="animate-spin rounded-full h-6 w-6 border-2 border-blue-200 border-t-blue-500" />
                  <span className="text-xs text-slate-400">Cargando reporte...</span>
                </div>
              ) : (
                <div className="px-2 sm:px-4 pb-2 max-w-5xl mx-auto w-full">
                  <DoubleBarChart
              ...
              <div className="px-4 sm:px-5 py-3 border-t border-slate-100 text-xs text-slate-500">
                <span className="font-semibold text-slate-700">
                  {ingresosGrowth >= 0 ? 'Tendencia al alza' : 'Tendencia a la baja'}.
                </span>{' '}
                Los ingresos {ingresosGrowth >= 0 ? 'subieron' : 'bajaron'} {Math.abs(ingresosGrowth).toFixed(1)}% frente al año anterior.
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
              <div className="px-4 sm:px-5 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100">
                <div>
                  <h3 className="text-sm font-semibold text-slate-800">Facturación y costos</h3>
                  <p className="text-xs text-slate-500 mt-0.5">
                    Ventas vs gastos · {formatPeriodLabel(desde, hasta)}
                  </p>
                </div>
                <div className="relative shrink-0">
                  <select
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-slate-100 border-b border-slate-100">
                <div className="px-4 sm:px-5 py-4">
                  <span className="text-xs text-slate-500 font-medium">Ventas totales</span>
                  <p className="text-lg font-bold text-slate-800 mt-1">{fmt(ventasTotal)}</p>
                  {renderGrowthIndicator(ventasGrowth, 'text-blue-600')}
                </div>
                <div className="px-4 sm:px-5 py-4">
                  <span className="text-xs text-slate-500 font-medium">Gastos totales</span>
                  <p className="text-lg font-bold text-slate-800 mt-1">{fmt(gastosTotal)}</p>
                  {renderGrowthIndicator(gastosGrowth, 'text-rose-600')}
                </div>
                <div className="px-4 sm:px-5 py-4">
                  <span className="text-xs text-slate-500 font-medium">Utilidad bruta</span>
                  <p className="text-lg font-bold text-slate-800 mt-1">{fmt(utilidadTotal)}</p>
                  {renderGrowthIndicator(utilidadGrowth, 'text-slate-600')}
                </div>
              </div>
              ...
              <div className="px-4 sm:px-5 py-3 border-t border-slate-100 text-xs text-slate-500">
                <span className="font-semibold text-slate-700">
                  {ventasGrowth >= 0 ? 'Ventas al alza' : 'Ventas a la baja'}.
                </span>{' '}
                Variación de {Math.abs(ventasGrowth).toFixed(1)}% respecto al año anterior.
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'ventas' && (() => {
          ...
          canalVentas = [
            ...
            { label: 'VENDEDORES', value: activeSource.VENDEDORES?.ventas || 0, color: '#2563eb' }
          ];

          let metodoIngresos = [];
          const activeMethods = activeWeekly.ingresosMetodo || {};
          const METHOD_COLORS = ['#2563eb', '#059669', '#d97706', '#0f766e', '#e11d48', '#0284c7', '#65a30d', '#b91c1c', '#1d4ed8'];
            ...
            <div className="space-y-6 p-4 animate-fadeIn">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-white p-6 rounded-xl border border-slate-200 flex flex-col justify-between">
                  <div>
                    <div className="mb-4">
                      <h3 className="text-sm font-semibold text-slate-800">Ventas del periodo por semana</h3>
                      <div className="flex items-center gap-2 mt-1">
                        <p className="text-xs text-slate-500">Distribución de ventas semanales</p>
                        <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded bg-slate-50 border border-slate-200/60 text-slate-500 uppercase tracking-wider">
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-100 text-xs text-slate-500">
                    Mejor semana: <span className="font-semibold text-slate-700">{bestWeekName}</span> con{' '}
                    <span className="font-semibold text-slate-700">${(bestWeekVal / 1000).toFixed(1)}k</span> en ventas.
                  </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-200 flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start gap-3 mb-4">
                      <div>
                        <h3 className="text-sm font-semibold text-slate-800">Ventas por mes del año</h3>
                        <div className="flex items-center gap-2 mt-1">
                          <p className="text-xs text-slate-500">Comportamiento anual acumulado</p>
                          <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded bg-slate-50 border border-slate-200/60 text-slate-500 uppercase tracking-wider">
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-100 text-xs text-slate-500">
                    Mejor mes: <span className="font-semibold text-slate-700">{bestMonthFullName}</span> con{' '}
                    <span className="font-semibold text-slate-700">${(bestMonthVal / 1000).toFixed(0)}k</span> en ventas.
                  </div>
                </div>
              </div>
            </div>
          ...
          return (
            <div className="space-y-6 p-4 animate-fadeIn">
              <div className="bg-white p-5 rounded-xl border border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h3 className="text-sm font-semibold text-slate-800">Cuentas por cobrar (Clientes)</h3>
                  <p className="text-xs text-slate-500 mt-0.5">Detalle de facturación pendiente y saldos por cobrar.</p>
                </div>
                <div className="text-left sm:text-right">
                  <span className="block text-xs text-slate-500 font-medium">Saldo por cobrar</span>
                  <span className="text-lg font-bold text-slate-800 block mt-0.5">{fmt(pendienteTotal)}</span>
                </div>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
                <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-slate-200 flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start gap-3 mb-4">
                      <div>
                        <h3 className="text-sm font-semibold text-slate-800">Saldos pendientes por cobrar por mes</h3>
                        <div className="flex items-center gap-2 mt-1">
                          <p className="text-xs text-slate-500">Montos pendientes de cobro por mes</p>
                          <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded bg-slate-50 border border-slate-200/60 text-slate-500 uppercase tracking-wider">
                            {formatPeriodLabel(desde, hasta)}
                          </span>
                        </div>
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-100 text-xs text-slate-500">
                    El saldo pendiente de <span className="font-semibold text-slate-700">{maxMonthFullName}</span> es el más alto del año con{' '}
                    <span className="font-semibold text-slate-700">{fmt(maxMonthVal)}</span>.
                  </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-200 flex flex-col justify-between">
                  <div>
                    <div className="mb-6">
                      <h3 className="text-sm font-semibold text-slate-800">Proporción de cobro</h3>
                      <div className="flex items-center gap-2 mt-1">
                        <p className="text-xs text-slate-500">Ratio de facturación liquidada</p>
                        <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded bg-slate-50 border border-slate-200/60 text-slate-500 uppercase tracking-wider">
                          {formatPeriodLabel(desde, hasta)}
                        </span>
                      </div>
                      ...
                      <div className="space-y-3 flex-1 w-full text-xs">
                        <div className="flex items-center justify-between border-b border-slate-50 pb-2">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-semibold text-slate-400 uppercase tracking-wider text-[9px]">Cobrado</span>
                          </div>
                          <span className="font-bold text-slate-800">{fmt(cobradoTotal)}</span>
                        </div>
                        <div className="flex items-center justify-between pb-1">
                          <div className="flex items-center gap-2">
                            <div className="w-2.5 h-2.5 rounded-full bg-slate-200 shrink-0" />
                            <span className="font-semibold text-slate-400 uppercase tracking-wider text-[9px]">Pendiente</span>
                          </div>
                          <span className="font-bold text-slate-800">{fmt(pendienteTotal)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="pt-2 border-t border-slate-100 text-xs text-slate-500">
                      Cobrado el <span className="font-semibold text-slate-700">{Math.round(pctCobrado)}%</span> de la facturación acumulada.
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white p-6 rounded-xl border border-slate-200">
                <div className="flex items-center gap-2 mb-2">
                  <h4 className="text-sm font-semibold text-slate-800">Detalle por proforma y cliente</h4>
                  <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded bg-slate-50 border border-slate-200/60 text-slate-500 uppercase tracking-wider">
                    {formatPeriodLabel(desde, hasta)}
                  </span>
                </div>
                        ...
                        proformasFiltered.map((item, idx) => {
                          ...
                          return (
                            <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                              ...
                              <td className="py-3.5 text-right text-amber-600 font-semibold">{fmt(item.pendiente)}</td>
                              <td className="py-3.5 text-center pr-2">
                                <span className={`inline-block text-[10px] font-bold uppercase px-2.5 py-0.5 rounded-full border ${badgeBg}`}>
                                  {statusText}
                                </span>
                              </td>
                            </tr>
                ...
                <div className="mt-4 pt-4 border-t border-slate-100 flex justify-center">
                  <button className="text-xs font-semibold text-slate-600 hover:text-slate-800 transition-colors">
        ...
        {activeTab === 'gastos' && (() => {
          ...
          const colorsDev = {
            ...
            'Redes y Programas': 'bg-teal-600 text-teal-600',
            'Vehículos': 'bg-blue-600 text-blue-600',
            'Logística': 'bg-sky-600 text-sky-600',
            'Varios': 'bg-slate-500 text-slate-500',
          ...
          const eColors = ['#d97706', '#e11d48', '#0284c7', '#059669', '#0f766e', '#475569'];
          ...
          return (
            <div className="space-y-6 p-4 animate-fadeIn">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-200 h-full">
                  <div>
                    {/* Header */}
                    <div className="mb-6">
                      <h3 className="text-sm font-semibold text-slate-800">Distribución de Gastos Devengados</h3>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded bg-slate-50 border border-slate-200/60 text-slate-500 uppercase tracking-wider">
                          {MONTH_FULL_NAMES[selectedHeaderMonth]} {desde.split('-')[0]}
                        </span>
                    ...
                    <div className="mb-6">
                      ...
                      <h2 className="text-2xl font-bold text-slate-800 mt-1">{fmt(devengadosTotal)}</h2>
                      <span className="text-[9px] font-semibold text-slate-400 block mt-0.5">100% del total</span>
                    </div>
                      ...
                      {devengadosList.map((item, idx) => {
                        ...
                        return (
                          <div key={idx} className="flex items-center gap-4 text-xs font-medium text-slate-700">
                            ...
                            {/* Label */}
                            <span className="w-28 truncate font-semibold text-slate-500 uppercase tracking-wider text-[10px]">{item.label}</span>
                            ...
                            <span className="w-10 text-right font-bold text-slate-800">{item.pct}%</span>
                            <span className="w-20 text-right font-semibold text-slate-500">{fmt(item.value)}</span>
                          </div>
                        );
                      })}
                    </div>
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-200 h-full">
                  <div>
                    {/* Header */}
                    <div className="mb-6">
                      <h3 className="text-sm font-semibold text-slate-800">Distribución de Egresos Reales (Desembolsos)</h3>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded bg-slate-50 border border-slate-200/60 text-slate-500 uppercase tracking-wider">
                          {MONTH_FULL_NAMES[selectedHeaderMonth]} {desde.split('-')[0]}
                        </span>
                    ...
                    <div className="mb-6">
                      ...
                      <h2 className="text-2xl font-bold text-slate-800 mt-1">{fmt(egresosTotal)}</h2>
                      <span className="text-[9px] font-semibold text-slate-400 block mt-0.5">100% del total</span>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-8 justify-center min-h-[360px]">
                      ...
                      <div className="relative w-44 h-44 shrink-0">
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none text-center">
                          <span className="text-[8px] text-slate-400 font-semibold uppercase tracking-widest">Total</span>
                          <span className="text-base font-bold text-slate-800 leading-none mt-0.5">
                        ...
                        {egresosList.map((item, idx) => {
                          ...
                          return (
                            <div key={idx} className="flex items-center justify-between py-2.5 border-b border-slate-50 last:border-none">
                              ...
                              <div className="text-right flex items-center gap-3">
                                <span className="font-semibold text-slate-805">{pct}%</span>
                                <span className="font-bold text-slate-500 w-16 text-right">${Math.round(item.value).toLocaleString()}</span>
                              </div>
                            </div>
                          );
                        })}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-200 flex flex-col justify-between h-full">
                  <div>
                    {/* Header */}
                    <div className="mb-4">
                      <h3 className="text-sm font-semibold text-slate-800">Gastos Totales por Semana</h3>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded bg-slate-50 border border-slate-200/60 text-slate-500 uppercase tracking-wider">
                          {MONTH_FULL_NAMES[selectedHeaderMonth]} {desde.split('-')[0]}
                        </span>
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-200 h-full">
                  <div>
                    <div className="mb-6">
                      <h3 className="text-sm font-semibold text-slate-800">Nómina Liquidada por Empleado</h3>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded bg-slate-50 border border-slate-200/60 text-slate-500 uppercase tracking-wider">
                          {MONTH_FULL_NAMES[selectedHeaderMonth]} {desde.split('-')[0]}
                        </span>
                        ...
                        return (
                          <div key={idx} className="space-y-1">
                            <div className="flex justify-between text-xs font-semibold uppercase tracking-wider text-slate-500">
                              ...
                              <span className="text-slate-850 font-bold">{fmt(item.value)}</span>
                            </div>
                            <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                              <div style={{ width: `${pctWidth}%` }} className="bg-blue-600 h-full rounded-full" />
                            </div>
                          </div>
                        );
                      })}
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                    <span className="font-semibold text-blue-600 uppercase tracking-wide">Aportes acumulados IESS (Seguro Social):</span>
                    <span className="font-bold text-blue-900 bg-white border border-blue-200/60 px-4 py-1.5 rounded-xl shadow-sm text-sm">{fmt(iessTotal)}</span>
                  </div>
                </div>
            ...
            <div className="space-y-6 p-4 animate-fadeIn">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
                ...
                <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-slate-200 h-full">
                  <div>
                    <div className="mb-6">
                      <h3 className="text-sm font-semibold text-slate-800">Calificaciones de Satisfacción del Cliente</h3>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded bg-slate-50 border border-slate-200/60 text-slate-500 uppercase tracking-wider">
                          {MONTH_FULL_NAMES[selectedHeaderMonth]} {desde.split('-')[0]}
                        </span>
                      ...
                      <div className="flex-1 w-full space-y-6">
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center justify-between text-xs">
                            <span className="font-medium text-slate-600">Satisfechos (4-5)</span>
                            <div className="text-right flex flex-col">
                              <span className="font-bold text-slate-800 text-sm leading-tight">{displaySatPct}%</span>
                              <span className="text-[9px] font-medium text-slate-400">({satVal})</span>
                            </div>
                          </div>
                          <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                            <div style={{ width: `${displaySatPct}%` }} className="bg-emerald-600 h-full rounded-full" />
                          </div>
                        </div>

                        <div className="space-y-2">
                          <div className="flex items-center justify-between text-xs">
                            <span className="font-medium text-slate-600">Neutros (3)</span>
                            <div className="text-right flex flex-col">
                              <span className="font-bold text-slate-800 text-sm leading-tight">{displayNeuPct}%</span>
                              <span className="text-[9px] font-medium text-slate-400">({neuVal})</span>
                            </div>
                          </div>
                          <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                            <div style={{ width: `${displayNeuPct}%` }} className="bg-slate-400 h-full rounded-full" />
                          </div>
                        </div>

                        <div className="space-y-2">
                          <div className="flex items-center justify-between text-xs">
                            <span className="font-medium text-slate-600">Inconformes (1-2)</span>
                            <div className="text-right flex flex-col">
                              <span className="font-bold text-slate-800 text-sm leading-tight">{displayIncPct}%</span>
                              <span className="text-[9px] font-medium text-slate-400">({incVal})</span>
                            </div>
                          </div>
                          <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-100 text-xs text-slate-500">
                    La mayoría de los clientes están <span className="font-semibold text-slate-700">satisfechos</span>
                    {displaySatPct > 0 ? ` (${displaySatPct}%)` : ''}.
                  </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-xl border border-slate-200 h-full">
                  <div>
                    <div className="mb-6">
                      <h3 className="text-sm font-semibold text-slate-800">Eficiencia de Entregas</h3>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded bg-slate-50 border border-slate-200/60 text-slate-500 uppercase tracking-wider">
                          {MONTH_FULL_NAMES[selectedHeaderMonth]} {desde.split('-')[0]}
                        </span>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center justify-between p-3 rounded-xl border border-slate-200">
                        <span className="text-xs text-slate-500 font-medium">Trabajos en curso</span>
                        <span className="text-sm font-bold text-slate-800">{activeCurso} proyectos</span>
                      </div>

                      {/* Entregados fuera de tiempo */}
                      <div className="flex items-center justify-between p-3 rounded-xl border border-slate-200">
                        <span className="text-xs text-slate-500 font-medium">Entregados fuera de tiempo</span>
                        <span className="text-sm font-bold text-slate-800">{activeFuera} proyectos</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center justify-between text-xs pt-1">
                      <span className="text-slate-500 font-medium">Clientes auditados</span>
                      <span className="font-bold text-slate-800 text-sm">{totalVal}</span>
                    </div>
                  </div>
                </div>
        ...
        {activeTab === 'pagar' && (() => {
          ...
          return (
            <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm space-y-8 animate-fadeIn">
              {/* Header and top pink card */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h2 className="text-sm font-semibold text-slate-800">Cuentas por pagar mensual (compras)</h2>
                  <div className="flex items-center gap-2 mt-1">
                    <p className="text-slate-500 text-xs">Cumplimiento y liquidación de deudas con proveedores.</p>
                    <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded bg-slate-50 border border-slate-200/60 text-slate-500 uppercase tracking-wider">
                      {formatPeriodLabel(desde, hasta)}
                    </span>
                  </div>
                ...
                <div className="text-left sm:text-right shrink-0">
                  <span className="block text-xs text-slate-500 font-medium">Saldo por pagar</span>
                  <span className="text-lg font-bold text-slate-800 leading-tight mt-0.5 block">{fmt(pagarTotal)}</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <div className="p-4 rounded-xl bg-white border border-slate-200">
                  <span className="block text-xs text-slate-500 font-medium">Total comprometido (OC)</span>
                  <span className="text-base font-bold text-slate-800 mt-1 block">{fmt(compTotal)}</span>
                </div>

                {/* 2. Egresado (Abonado) */}
                <div className="p-4 rounded-xl bg-white border border-slate-200">
                  <span className="block text-xs text-slate-500 font-medium">Egresado (abonado)</span>
                  <span className="text-base font-bold text-slate-800 mt-1 block">{fmt(egrTotal)}</span>
                </div>

                {/* 3. Saldo Pendiente */}
                <div className="p-4 rounded-xl bg-white border border-slate-200">
                  <span className="block text-xs text-slate-500 font-medium">Saldo pendiente</span>
                  <span className="text-base font-bold text-slate-800 mt-1 block">{fmt(pendTotal)}</span>
                </div>

                {/* 4. Cumplimiento Promedio */}
                <div className="p-4 rounded-xl bg-white border border-slate-200">
                  <span className="block text-xs text-slate-500 font-medium">Cumplimiento promedio</span>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-base font-bold text-slate-800">{avgCumplimiento}%</span>
                    <div className="flex-1 bg-slate-100 h-1.5 rounded-full overflow-hidden">
                      <div style={{ width: `${avgCumplimiento}%` }} className="bg-slate-600 h-full rounded-full" />
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center justify-center gap-6 text-[10px] font-semibold text-slate-500 uppercase tracking-widest pt-2">
                <div className="flex items-center gap-2">
                ...
                <table className="w-full text-center text-xs border-collapse">
                  <thead>
                    <tr className="border-b border-slate-100 bg-slate-50/50 text-slate-400 font-bold uppercase tracking-wider">
                      <th className="py-3 px-4 text-left font-bold text-[10px] w-48 shrink-0">Categoría / Serie</th>
                      {breakdownList.map(b => (
                        <th key={b.label} className="py-3 px-2 font-bold text-[10px]">{b.label}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50 font-medium text-slate-650">
                    <tr className="hover:bg-slate-50/50 transition-colors">
                      <td className="py-3.5 px-4 text-left font-semibold text-[10px] uppercase tracking-wider text-slate-500 flex items-center gap-2">
