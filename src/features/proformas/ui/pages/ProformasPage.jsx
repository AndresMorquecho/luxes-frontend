                      ...
                      <tr key={p.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-5 py-3">
                          <div className="flex items-center justify-end gap-1">
                            ...
                            {/* Only pending proformas can be edited or deleted */}
                            {p.estado === 'Pendiente' && (
                              <>
                                <button 
                                  onClick={() => openEdit(p)} 
                ...
                {proformas.map(p => (
                  <div key={p.id} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm flex flex-col gap-3">
                    ...
                    <div className="flex justify-between items-center mt-1">
                      ...
                      <div>
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${badgeStyle(p.estado === 'Pagada' ? 'Aprobada' : p.estado)}`}>
                          {p.estado === 'Pagada' ? 'Aprobada' : p.estado}
                        </span>
                      </div>
                    </div>
                    <div className="flex items-center justify-end gap-2 mt-2 pt-2 border-t border-slate-100">
                      ...
                      {p.estado === 'Pendiente' && (
                        <>
                          ...
                          <button 
                            onClick={() => handleDelete(p.id)}
                            className="p-2 rounded-lg border border-red-100 hover:bg-red-50 text-slate-400 hover:text-red-600 transition-colors" 
                            title="Eliminar"
                          >
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                              <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
