  ...
  return (
    <div className="min-h-screen w-full bg-slate-50 text-slate-800 p-6 flex flex-col gap-6" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          { label: 'Horas trabajadas hoy', value: kioskSession?.lapsos?.trabajo || '0h 00m', icon: (
            <svg className="w-5 h-5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
            </svg>
          )},
          { label: 'Horas extras', value: kioskSession?.horasExtra?.horas ? `${kioskSession.horasExtra.horas}h` : '0h 00m', icon: (
            <svg className="w-5 h-5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
            </svg>
          )},
          { label: 'Marcaciones completadas', value: `${kioskSession?.marcaciones?.length || 0} de 4`, icon: (
            <svg className="w-5 h-5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2M9 5a2 2 0 0 0 2 2h2a2 2 0 0 0 2-2M9 5a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2" />
            </svg>
          )},
          { label: 'Estado de hoy', value: kioskSession ? 'En jornada' : 'Sin iniciar', isBadge: true, icon: (
            <svg className="w-5 h-5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
            </svg>
          )},
          { label: 'Ubicación actual', value: gpsBadge.tone === 'emerald' ? 'Oficina Central' : 'Sin señal GPS', icon: (
            <svg className="w-5 h-5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div key={idx} className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm flex items-center gap-3">
            ...
            <div>
              ...
              {item.isBadge ? (
                <span className={`inline-block mt-1 text-xs font-bold px-2 py-0.5 rounded-md border ${
                  kioskSession ? 'text-emerald-700 bg-emerald-50 border-emerald-100' : 'text-slate-500 bg-slate-50 border-slate-100'
                }`}>
                  {item.value}
                </span>
              ) : (
                ...
                {recentRegistros.map((reg) => (
                  <tr key={reg.id} className="hover:bg-slate-50/50">
                    ...
                    <td className="py-2.5 px-3">
                      <span className="text-slate-400 font-semibold">📍 Oficina Central</span>
                    </td>
                  </tr>
                ))}
              </tbody>
